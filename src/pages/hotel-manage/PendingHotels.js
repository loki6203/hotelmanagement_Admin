...
function ProductPage() {
  ...
  const getAllHotel = async () => {
    const { data, status } = await axios.get("Hotel/list?status=Pending"+ (getCookie("role") == "Hotel" ? `&author_id=${getCookie("author_id")}` : ''));
  ...
  return (
    <Layout title="Pending Hotels" content="container">
        <Block.Head>
            <Block.HeadBetween>
            <Block.HeadContent>
                    <Block.Title tag="h2">Pending Hotels</Block.Title>
                    <nav>
                        <ol className="breadcrumb breadcrumb-arrow mb-0">
                          <li className="breadcrumb-item"><Link to="/home">Home</Link></li>
                          <li className="breadcrumb-item">Hotels</li>
                          <li className="breadcrumb-item active" aria-current="page">Pending Hotels</li>
                        </ol>
                    </nav>
                </Block.HeadContent>
                <Block.HeadContent>
