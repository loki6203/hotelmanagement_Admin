...
function AddPropertyType() {
    ...
    const onSubmit = async (formData) => {
        const { data, status } = id ? await axios.patch(`PropertyType/update/${id}`, image?.includes(kImageURL)?{
            title:formData.title,
            status:formData?.status,
        } :formData) : await axios.post('PropertyType/create', formData);
    ...
    const getAmentyDetailsById = async (id) => {
        const { data, status } = await axios.get(`PropertyType/view/${id}`);
        if (status == 200 || status == 201) {
    ...
    console.log(status)
    return (
        <Layout title="Add Property Type" content="container">
            <Block.Head>
                <Block.HeadBetween>
                    <Block.HeadContent>
                        <Block.Title tag="h2">{id ? "Edit" : "Add"} Property Type</Block.Title>
                        <nav>
                            <ol className="breadcrumb breadcrumb-arrow mb-0">
                                <li className="breadcrumb-item"><Link to="/home">Home</Link></li>
                                <li className="breadcrumb-item"><Link to="/amenities/all-amenities">Property Type</Link></li>
                                <li className="breadcrumb-item active" aria-current="/amenities/add-amenity">Add Property Type</li>
                            </ol>
                        </nav>
                    </Block.HeadContent>
                </Block.HeadBetween>
                                            ...
                                            <Row className="g-gs">
                                                ...
                                                <Col lg="6">
                                                    <Form.Group className="form-group">
                                                        <Form.Label htmlFor="productname">Property Type Name</Form.Label>
                                                        <div className="form-control-wrap">
                                                            <Form.Control type="text" id="productname" {...register("title")} placeholder="property type Name" />
